...
WEEPER_CLOSE_TASKS_AFTER - Через какое количество дней закрывать рассылку и отсылать последнее письмо
по умолчанию WEEPER_CLOSE_TASKS_AFTER = 5
Это означает, что 4 дня после дедлайна будут приходить письма с напоминанием, а на пятый - последнее письмо и задача закроется.
Если выставить WEEPER_CLOSE_TASKS_AFTER = 0 то напоминания будут приходить вечно
...
Закрывать таски можно тремя способами:
...
3) Вызовом метода close_task_by_user(user) у TaskDelivery
    Например:
    td = TaskDelivery.objects.all()[0]
    user = td.users.all()[0]
    td.close_task_by_user(user)
